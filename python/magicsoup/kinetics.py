...
class Kinetics:
    ...
    def __init__(
        ...
        # TODO: create new versions of mappings:
        # vmax_map: HashMap<usize, f64>,
        # hill_map: HashMap<usize, usize>,
        # km_map: HashMap<usize, f64>,
        # sign_map: HashMap<usize, bool>,
        # react_map: HashMap<usize, Vec<(usize, isize)>>, (mol_i, stoich_number)
        # transport_map: HashMap<usize, Vec<(usize, isize)>>, (mol_i, stoich_number)
        # effector_map: HashMap<usize, Vec<(usize, usize)>>, (mol_i, reg_number)
        self.vmax_map_rs = {i: d for i, d in enumerate(self.vmax_map.weights.tolist())}
        self.hill_map_rs = {
            i: int(d) for i, d in enumerate(self.hill_map.numbers.tolist())
        }
        self.km_map_rs = {i: d for i, d in enumerate(self.km_map.weights.tolist())}
        self.sign_map_rs = {
            i: d > 0 for i, d in enumerate(self.sign_map.signs.tolist())
        }

        self.reaction_map_rs: dict[int, list[tuple[int, int]]] = {}
        for i in range(len(self.reaction_map.M)):
    ...
    def set_cell_params(
        ...
        Vmax, Kmn, N, Nf, Nb, A, Kmr = _lib.calculate_cell_params(
            proteomes,
            n_prots,
            len(self.molecules),
            self.vmax_map_rs,
            ...
            self.km_map_rs,
            self.sign_map_rs,
            self.reaction_map_rs,
            self.transport_map_rs,
        ...
        # this effectively limits Ke around 1e38
        is_fwd = Ke >= 1.0
        Kmf = torch.where(is_fwd, Kmn, Kmn / Ke).clamp(_EPS, _MAX)
        Kmb = torch.where(is_fwd, Kmn * Ke, Kmn).clamp(_EPS, _MAX)
        ...
        # setting new params
        self.Vmax[cell_idxs] = Vmax  # (c,p)
        self.A[cell_idxs] = A  # (c,p,s)
        self.Kmr[cell_idxs] = Kmr_rdy  # (c,p,s)
        self.N[cell_idxs] = N  # (c,p,s)
        self.Nf[cell_idxs] = Nf  # (c,p,s)
        self.Nb[cell_idxs] = Nb  # (c,p,s)
        self.Ke[cell_idxs] = Ke  # (c,p)
        self.Kmf[cell_idxs] = Kmf  # (c,p)
        self.Kmb[cell_idxs] = Kmb  # (c,p)
